...
import { FaBook, FaPhone, FaMapMarkerAlt, FaCity, FaRuler, FaHome, FaFileAlt } from 'react-icons/fa';
import { collegeApi, distanceApi, getterFunction, posterFunction } from '../../Api'; // Adjust path as needed
...
const CollegeDetails = () => {
      ...
      if (navigator.geolocation) {
        navigator.permissions.query({ name: 'geolocation' }).then((result) => {
          if (result.state === 'granted') {
            ...
            alert('Location permission is required to calculate the distance. Please enable it in your browser settings.');
            resolve(false);
          }
        });
      } else {
  ...
  const { name, description, university, mobile, address, mainCity, mainCityDistance } =
    collegeData?.college;
        ...
        <div className="space-y-4">
          ...
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-gray-600">{(mainCityDistance / 1000).toFixed(2)} km</p>
            </div>
          </div>
          <div
          ...
          >
            ...
            <div>
              ...
              <p className="text-gray-600">
                {distance ? `${(distance / 1000).toFixed(0)} km` : 'Click to calculate distance'}
              </p>
            </div>
          </div>
        </div>
          ...
          collegeData.courses.map((course, index) => (
            <div key={course._id} className="mb-6">
              <div className="flex items-center gap-2">
                ...
                <div>
                  <p className="text-lg font-semibold text-gray-800">{course.title.toUpperCase()}</p>
                  <p className="text-gray-600">{course?.description ?? 'No Description Found'}</p>
                </div>
              </div>
              <div className="flex justify-between items-center mt-2">
                <p className="font-semibold">Eligibility:</p>
                <p className="text-gray-600">{course?.Eligibility || 'Not specified'}</p>
              </div>
              <div className="flex justify-between items-center mt-2">
                ...
                <button
                  ...
                  onClick={() =>
                    handleInquiryCall(collegeData?.supoorts[index]?.mobile || mobile)
                  }
