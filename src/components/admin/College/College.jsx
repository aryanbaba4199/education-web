...
import AddCollege from "./CollegeForm"; // Assuming this is the correct import
...
const CollegeList = () => {
  ...
  const handleEdit = (college) => {
    setEditMode(college); // Pass the entire college object
    setShowCollege(true); // Open the AddCollege form immediately
  };
                    ...
                    <ul className="list-disc list-inside text-gray-600 text-sm">
                      {college.courseIds.map((courseId, index) => (
                        <li key={courseId}>
                          ...
                          {college.fees?.[index] && ` - ₹ ${college.fees[index]}/-`}
                        </li>
                      ))}
                    </ul>
                  </div>
