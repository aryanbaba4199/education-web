      ...
      <Box className="max-w-7xl mx-auto">
        ...
        <div>
           ...
           <div className='flex flex-wrap gap-8 mb-8'>
            {filterButtons.map((item, index)=>(
                <button onClick={()=>handleFilter(item)} className={`px-4 ${activeFilter===item.value ?"bg-cyan-600" :"bg-slate-800"} hover:bg-slate-700 text-white rounded-md py-1`} key={item.value}>{item.title}</button>
            ))}
           </div>
        </div>
        {activeFilter===0 &&
        <div>
            <div className='flex justify-center items-center mb-8'>
                <Typography fontSize={18} fontWeight='700'>All Calls</Typography>
            </div>
            ...
            <TableBody>
              {filteredCalls.map((call, index) => (
                ...
                >
                  ...
                  <TableCell className='hover:cursor-pointer hover:bg-gray-300' onClick={()=>setSelectedCall(call._id)}><FaEye className='text-green-600'/></TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
